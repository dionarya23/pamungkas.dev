...
<div class="mx-auto w-full max-w-2xl space-y-8 bg-white print:space-y-6">
  <div class="space-y-3">
    ...
    <p class="text-gray-700">
      Writing about programming, development, and the ideas behind the code.<br
        ...
        <section class="space-y-6">
          ...
          <div class="space-y-6">
            {#each group.posts as post (post.slug)}
              <article
                class="border border-gray-200 rounded-lg p-6 transition-shadow group hover:shadow-sm"
              >
                <a href={`/blog/${post.slug}`} class="block">
                  <h3
                    class="text-2xl font-semibold mb-2 group-hover:underline text-black"
                  >
                    {post.title}
                  </h3>
